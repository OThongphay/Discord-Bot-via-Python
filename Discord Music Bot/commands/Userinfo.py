...
class Userinfo(commands.Cog):
    ...
    #If statement if there is
        ...
        info_embed.add_field(name = "Joined At:", value = member.joined_at.__format__("%A, %d. %B %Y @%H:%M:%S"), inline = False)
        info_embed.add_field(name = "Creation Date:", value = member.created_at.__format__("%A, %d. %B %Y @%H:%M:%S"), inline = False)
        info_embed.set_footer(text = f"Requested by <@{ctx.author}>.", icon_url = ctx.author.avatar)
